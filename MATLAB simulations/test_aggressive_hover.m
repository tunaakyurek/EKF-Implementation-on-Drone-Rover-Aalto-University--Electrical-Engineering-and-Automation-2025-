%% test_aggressive_hover.m - Aggressive Hover Test
% PURPOSE
% Test aggressive hover control with stronger gains to achieve stable flight.

function test_aggressive_hover()
    fprintf('=== Aggressive Hover Test ===\n\n');
    ...
    fprintf('Testing aggressive hover for %.1f seconds...\n', duration);
    
    for step = 1:steps
        t = step * dt;
        time(step) = t;
        
        % Aggressive hover control
        control_inputs(:, step) = compute_aggressive_hover_control(true_state, params);
    ...
    if position_error < 1.0
        fprintf('  ✅ Aggressive hover: SUCCESS\n');
    elseif position_error < 3.0
        fprintf('  ⚠️  Aggressive hover: PARTIAL SUCCESS\n');
    else
        fprintf('  ❌ Aggressive hover: NEEDS IMPROVEMENT\n');
    end
...
function control_input = compute_aggressive_hover_control(current_state, params)
    %% Aggressive Hover Control
    ...
    kp = [2.0, 2.0, 5.0];  % [x, y, z] gains - much stronger
    desired_vel = kp' .* pos_error;
    
    % Velocity error
    vel_error = desired_vel - current_vel;
    
    % Aggressive velocity control
    kv = [1.0, 1.0, 2.0];  % Velocity gains - much stronger
    ...
    % Convert to thrust with proper scaling
    thrust_magnitude = norm(desired_accel) * params.mass;
    thrust_magnitude = max(min(thrust_magnitude, 50.0), 10.0);  % 10-50 N range
    
    % Aggressive attitude control (level flight)
    target_att = [0; 0; 0];
    att_error = target_att - current_att;
    torque = att_error * 2.0;  % Stronger attitude control
...
function plot_hover_results(time, true_pos, control_inputs)
    ...
    sgtitle('Aggressive Hover Test', 'FontSize', 14);
    saveas(gcf, 'aggressive_hover_test.png');
    fprintf('Plot saved: aggressive_hover_test.png\n');
end
