function x_s = post_smooth_estimates_8state(t, x)
% POST_SMOOTH_ESTIMATES_8STATE - Offline spike suppression and zero-phase smoothing for 8-state rover
% Inputs:
%   t : 1xN time vector
%   x : 8xN state history (EKF output) [x, y, theta, vx, vy, b_ax, b_ay, b_w]
% Output:
%   x_s : 8xN smoothed state history
...
% 3) Wrap yaw (theta) back to [-pi,pi] - state 3
x_s(3,:) = atan2(sin(x_s(3,:)), cos(x_s(3,:)));
