%% test_expert_stable.m - Stable Expert System Test
% PURPOSE
% Test the expert flight controller with conservative, stable control parameters.

function test_expert_stable()
    fprintf('=== Stable Expert System Test ===\n\n');
    ...
    % Create expert flight controller with conservative settings
    expert_controller = expert_flight_controller(params);
    
    % Reduce control aggressiveness
    expert_controller.max_velocity = 1.0;        % Reduced from 2.0
    ...
    obstacle_map = map_gen.generate_column_obstacles(0.1);  % 10% density (fewer obstacles)
    
    % Define start and goal positions (closer together)
    start_pos = [-10; -10; 5];  % Start position
    goal_pos = [10; 10; 5];     % Goal position (same altitude)
    ...
    path_info_log = cell(1, steps);
    
    fprintf('Testing stable expert obstacle avoidance for %.1f seconds...\n', duration);
    ...
    for step = 1:steps
        ...
        path_info_log{step} = path_info;
        
        % Log true state
        true_pos(:, step) = true_state(1:3);
        
        % Conservative dynamics simulation
        true_state = simulate_conservative_dynamics(true_state, control_inputs(:, step), dt, params);
        ...
        % Safety check - stop if flying too far away
    ...
    elseif final_error < 10.0
        fprintf('  ⚠️  Expert system: PARTIAL SUCCESS\n');
    else
        fprintf('  ❌ Expert system: NEEDS IMPROVEMENT\n');
    end
    
    % Plot results
    plot_stable_results(time, true_pos, expert_pos, obstacle_map, start_pos, goal_pos, control_inputs);
end

function true_state = simulate_conservative_dynamics(current_state, control_inputs, dt, params)
    %% Conservative Drone Dynamics Simulation
    ...
    % Conservative velocity dynamics
    ...
    % Total acceleration with conservative scaling
    accel = gravity + thrust_world / params.mass;
    
    % Strong damping for stability
    damping = -0.5 * vel;  % Increased damping
    accel = accel + damping;
    
    new_vel = vel + accel * dt;
    
    % Conservative attitude dynamics
    if length(torque) == 3 && length(params.I) == 3
        ...
        att_rate = torque ./ I_diag * 0.5;  % Reduced gain
    else
        att_rate = torque / params.mass * 0.5;  % Reduced gain
    end
    
    % Strong attitude damping
    att_damping = -0.3 * att;  % Increased damping
...
function plot_stable_results(time, true_pos, expert_pos, obstacle_map, start_pos, goal_pos, control_inputs)
    %% Plot Stable Results
    ...
    sgtitle('Stable Expert System Test', 'FontSize', 14);
    saveas(gcf, 'stable_expert_test.png');
    fprintf('Plot saved: stable_expert_test.png\n');
end
